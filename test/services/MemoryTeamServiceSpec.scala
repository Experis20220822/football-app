...
class MemoryTeamServiceSpec extends PlaySpec {
  "MemoryTeamService" must {
    "return the size of the list after I create the team" in {
      ...
      val team = Team(12L, "Arsenal", Stadium(12L,"Emirates Stadium","London","UK", 60260))
    ...
    "find a team by an id and return that team if it is in there" in {
      val memoryTeamService = new MemoryTeamService()
      val arsenal = Team(12L, "Arsenal", Stadium(12L,"Emirates Stadium","London","UK", 60260))
      val chelsea = Team(10L, "Chelsea", Stadium(10L,"Stamford Bridge","London", "UK", 41837))
    ...
    "be able to update a team from the service" in {
      ...
      val updated = arsenal.copy(stadium = Stadium(12L, "Ashburton Grove","London", "UK", 60000))
      println(updated)
      memoryTeamService.update(updated)
      val result = memoryTeamService.findById(12L).get
      result.stadium mustBe Stadium(12L, "Ashburton Grove","London", "UK", 60000)
    ...
    "be able to update a team that is not in the service" in {
      ...
      memoryTeamService.create(arsenal)
      memoryTeamService.create(chelsea)
      val result = memoryTeamService.update(Team(30L, "Lewes FC", Stadium(30L,"The Dripping Pan","Lewes", "UK", 500)))
